...
<a class="navbar-brand" href="https://new.wwwhassanfalh5041.workers.dev/">موقع مجتهد</a>
...
<li class="nav-item">
  <a class="nav-link active" href="#">الرئيسيه<span class="visually-hidden">(current)</span>
  </a>
</li>
<li class="nav-item">
...
<li class="nav-item dropdown">
  <a class="nav-link dropdown-toggle" data-bs-toggle="dropdown" href="#" role="button" aria-haspopup="true" aria-expanded="false">قائمه المشاركه</a>
  <div class="dropdown-menu">
<ul class="navbar-nav me-auto mb-2 mb-lg-0" style="
">
                <li class="nav-item">
                    <a href="https://www.facebook.com/sharer/sharer.php?u=YOUR_URL_HERE" target="_blank" rel="noopener noreferrer" class="nav-link" style="
color: black;
font-size: medium;
display: flex;
flex-direction: row-reverse;
align-items: center;
">مشاركه على الفيس بوك
...
<a class="nav-link" href="tg://msg_url?url=YOUR_URL_HERE" target="_blank" rel="noopener noreferrer" style="color: black;font-size: medium;display: flex;flex-direction: row-reverse;align-items: center;">مشاركه على التليكرام
...
function requestListPath(e, t, n, a) {
    ...
    $.post(e, i, (function(t, l) {
        ...
        sleep(2e3),
        $("#update").html("<div class='alert alert-info' role='alert'> اعد المحاوله ...</div></div></div>"),
        $.post(e, i, (function(t, l) {
            var o = jQuery.parseJSON(gdidecode(read(t)));
            o && o.error && "401" == o.error.code ? a && a(e) : o && null === o.data ? ($("#spinner").remove(),
            $("#list").html("<div class='alert alert-danger' role='alert'> Server didn't sent any data. </div></div></div>"),
...
function file_pdf(e) {
    ...
    $.post("", (function(e) {
        try {
            var t = jQuery.parseJSON(gdidecode(read(e)))
              , i = formatFileSize(t.size)
              , l = `<script> var url = "https://" + window.location.hostname + window.location.pathname;  var pdfjsLib = window['pdfjs-dist/build/pdf']; pdfjsLib.GlobalWorkerOptions.workerSrc = '//cdn.jsdelivr.net/npm/pdfjs-dist@2.12.313/build/pdf.worker.js'; var pdfDoc = null, pageNum = 1, pageRendering = false, pageNumPending = null, scale = 0.8, canvas = document.getElementById('the-canvas'), ctx = canvas.getContext('2d');  function renderPage(num) { pageRendering = true; pdfDoc.getPage(num).then(function(page) { var viewport = page.getViewport({scale: scale}); canvas.height = viewport.height; canvas.width = viewport.width; var renderContext = { canvasContext: ctx, viewport: viewport }; var renderTask = page.render(renderContext); renderTask.promise.then(function() { pageRendering = false; if (pageNumPending !== null) { renderPage(pageNumPending); pageNumPending = null; } }); }); document.getElementById('page_num').textContent = num; } function queueRenderPage(num) { if (pageRendering) { pageNumPending = num; } else { renderPage(num); } }  function onPrevPage() { if (pageNum <= 1) { return; } pageNum--; queueRenderPage(pageNum); } document.getElementById('prev').addEventListener('click', onPrevPage);  function onNextPage() { if (pageNum >= pdfDoc.numPages) { return; } pageNum++; queueRenderPage(pageNum); } document.getElementById('next').addEventListener('click', onNextPage);  pdfjsLib.getDocument(url).promise.then(function(pdfDoc_) { pdfDoc = pdfDoc_; document.getElementById('page_count').textContent = pdfDoc.numPages; renderPage(pageNum); }); </script> <div class="container"><br> <div class="card"> <div class="card-body text-center"> <!-- عرض تفاصيل الملف --> <div class="${UI.file_view_alert_class}" id="file_details" role="alert">${t.name}<br>${i}</div> <div class="card-body"> <div class="input-group mb-4"> <div class="card-text text-center">  ${UI.display_drive_link ? '<a type="button" class="btn btn-info" href="https://drive.google.com/file/d/' + t.id + '/view" id ="file_drive_link" target="_blank">GD Link</a>' : ""} <div class="btn-group text-center"> <a href="${a}" type="button" class="btn btn-primary">Download</a> <br>
          ...
          <p>الصفحة الخاصه بتحميل النتائج</p>
          
              <div class="text-center mt-5">
                  <button onclick="goToNewLink()" class="btn btn-primary">اضغط لتوليد رابط تحميل لك</button>
                  <br>
                  <span id="countdown-timer"></span>
                  <br>
                  <br>
                  <div class="alert alert-dismissible alert-success d-none mt-5" id="download-alert" style="margin-top: 0rem !important;">
                      <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                      <strong>تمام!</strong> لقد تم توليد رابط لك في اسفل الصفحة 
                  </div>
                  
                  <br>
                  <p>نتظر الرابط</p>
                  <a  class="alert-success d-none" href="${a}" id="original-link">حمل الان ....</a>
              </div>
              
              <script>
                  var originalLink = $('#original-link').attr('href');
                  $('#original-link').removeAttr('href');
          
                  function goToNewLink() {
                      var timer = 1;
                      var btn = document.getElementsByTagName("button")[0];
                      var message = document.getElementById("download-alert");
                      var downloadLink = document.getElementById("original-link");
                      var countdownTimer = document.getElementById("countdown-timer");
                      btn.disabled = true;
                      countdownTimer.innerText = "انتظر " + timer + " ثانية...";
                      var countDown = setInterval(function(){
                          if (timer <= 0) {
                              ...
                              downloadLink.setAttribute('href', originalLink);
                          } else {
                              countdownTimer.innerText = "انتظر " + timer + " ثانية...";
          if (timer === 0) {
